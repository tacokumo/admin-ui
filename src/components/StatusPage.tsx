		...
		<Stack gap="md">
			<Group justify="space-between" align="center">
				...
				<Button
					...
					onClick={checkAll}
					loading={
						health.liveness.status === "loading" ||
						health.readiness.status === "loading"
					}
				>
					Check All
				</Button>
			...
			<Alert
				...
				color="blue"
			>
				This page tests the useAdminAPI hook by making authenticated requests to
				the health endpoints. The hook automatically includes Auth0 Bearer
				...
				<Card shadow="sm" padding="lg" radius="md" withBorder>
					<Stack gap="sm">
						...
						{health.liveness.message && (
							<Text
								size="sm"
								c={health.liveness.status === "error" ? "red" : "green"}
							>
								{health.liveness.message}
							</Text>
						)}
				...
				<Card shadow="sm" padding="lg" radius="md" withBorder>
					<Stack gap="sm">
						...
						{health.readiness.message && (
							<Text
								size="sm"
								c={health.readiness.status === "error" ? "red" : "green"}
							>
								{health.readiness.message}
							</Text>
						)}
